...
function ColorPreviewIcon({ hexValue }: { hexValue: string }): Image.ImageLike {
  console.log(`ColorPreviewIcon using hexValue: ${hexValue}`);
  ...
  return (
    ...
    >
      {convertedColors.map(({ format, value, hexValue }, index) => {
        console.log(`Calling ColorPreviewIcon with format: ${format}, colorValue: ${value}, hexValue: ${hexValue}`);
        const icon = ColorPreviewIcon({ hexValue });
        return (
          <List.Item
            key={index}
            icon={icon}
            title={format === "figma" ? "Figma P3" : format.toUpperCase()}
            subtitle={value}
            accessories={[
              {
